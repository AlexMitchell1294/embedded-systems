...
unsigned char getPressed(){
    ...
    if((P3IN & (BIT6)) == 0) ret_val = led3;
    else if((P7IN & (BIT4)) == 0) ret_val = led1;
    else if((P7IN & (BIT0)) == 0) ret_val = led4;
    else if((P2IN & (BIT2)) == 0) ret_val = led2;
    else ret_val = 0x00;

    return ret_val;
}

void turnLedsOn(unsigned char inBits){
        unsigned char mask = 0;

        // Turn all LEDs off to start
        P6OUT &= ~(BIT4|BIT3|BIT2|BIT1);

        if (inBits & BIT0) mask |= BIT4;   // Right most LED P6.4
        if (inBits & BIT1) mask |= BIT3;   // next most right LED P.3
        if (inBits & BIT2) mask |= BIT1;   // third most left LED P6.1
        if (inBits & BIT3) mask |= BIT2;   // Left most LED on P6.2
        P6OUT |= mask;
...
void playNote(int value)
{
    ...
    TB0CCR0   = 32768/value;//32768/value;                    // Set the PWM period in ACLK ticks
